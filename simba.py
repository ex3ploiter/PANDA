...
class SimBA:
    ...
    def simba_batch(self, images_batch, labels_batch, max_iters, freq_dims, stride, epsilon, linf_bound=0.0,
        ...
        for k in range(max_iters):
            dim = indices[k]
            #print("")
            #print(images_batch.device, x.device, self.expand_vector(x[remaining_indices], expand_dims).device, trans(self.expand_vector(x[remaining_indices], expand_dims)).device)
            ...
            if remaining.sum() == 0:
                ...
                break
            remaining_indices = torch.arange(0, batch_size)[remaining.cpu()].long().cuda()
            if k > 0:
                succs[:, k] = ~remaining
            diff = torch.zeros(remaining.sum(), n_dims).cuda()
            diff[:, dim] = epsilon
